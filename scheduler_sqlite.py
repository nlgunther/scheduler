#!/usr/bin/env python3
"""
scheduler_sqlite.py - Hybrid SQLite + JSON Sidecar Storage Engine.
"""
import sqlite3
import json
import shutil
...
from scheduler import (
    ...
    ModelEncoder, task_from_dict, contact_from_dict
)

# FIX: Import config to get default path
try:
...
class SqliteStorage(StorageStrategy):
    """Hybrid storage: Metadata in SQLite, Content in JSON Sidecars."""

    def __init__(self, base_dir: Optional[Path] = None):
        # FIX: Resolve default path immediately
        path = base_dir if base_dir else get_data_dir()
    ...
    def rename_project(self, old_slug: str, new_slug: str) -> None:
        """
        Renames project transactionally.
        1. Create new project row (clone of old)
        2. Re-parent tasks and contacts to new slug
        3. Delete old project row
        4. Rename sidecar directory
        """
        with self._get_conn() as conn:
            # 1. Clone Project
            ...
            # 3. Delete Old Project (Safe now that children are moved)
